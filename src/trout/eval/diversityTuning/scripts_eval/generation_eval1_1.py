...
from trout.eval.diversityTuning.models import (
    GenerationModel,
    CreativeWritingRewardModel,
    DiversityModel,
    OpenAIModel,
    AnthropicModel,
    TogetherModel,
...
# add the arguments
parser.add_argument(
    "--modelname", default="meta-llama/Llama-3.1-8B", type=str, help="Model name"
)
parser.add_argument(
    "--lora_model_path",
    default="checkpoints/Llama-3.1-8B/generation_models_DDPO_sem_sty/ddpo_00.pt",
    type=str,
    help="Path to the model",
)
parser.add_argument(
    "--val_path",
    default="data/eval/ablation/gen_eval1_DDPO_sem_sty_.csv",
    type=str,
...
parser.add_argument(
    "--rewardmodelname", default="google/gemma-2-2b", type=str, help="Reward model name"
)
parser.add_argument(
    "--reward_lora_model_path",
    default="checkpoints/reward_models/google_gemma-2-2b_10.pt",
    type=str,
    help="Path to the reward model",
)

parser.add_argument(
    "--semanticdivmodelname",
    default="jinaai/jina-embeddings-v3",
    type=str,
    help="Diversity model name",
)
parser.add_argument(
    "--styledivmodelname",
...
reward_model = CreativeWritingRewardModel(
    args.rewardmodelname,
    lora_model_path=args.reward_lora_model_path,
    device=args.device,
)
...
for i in tqdm(range(len(val_dataset))):
    ...
    if (
        "gpt" in args.modelname
        or "o1" in args.modelname
        or "claude" in args.modelname
        or "deepseek" in args.modelname
    ):
    ...
    outputs_for_rm = [
        f"<prompt>{prompt}</prompt><response>{output}</response>" for output in outputs
    ]
