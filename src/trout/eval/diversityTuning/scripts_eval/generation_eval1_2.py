...
from trout.eval.diversityTuning.diversity import (
    compression_ratio,
...
# add the arguments
parser.add_argument(
    "--val_path", default="data/eval", type=str, help="Path to the dataset"
)
parser.add_argument("--device", default="cuda:1", type=str, help="Device to use")
parser.add_argument(
    "--val_files", default=[], nargs="+", type=str, help="List of val files"
)
...
for val_file in val_files:
    ...
    if (
        "compression_ratio" not in val_df.columns
        and "homogenization_score" not in val_df.columns
        and "ngram_diversity_score" not in val_df.columns
    ):
    ...
    if (
        val_df["compression_ratio"].isnull().values.any()
        or val_df["homogenization_score"].isnull().values.any()
        or val_df["ngram_diversity_score"].isnull().values.any()
    ):
        for prompt_id in val_df["prompt_id"].unique():
            ...
            val_df.loc[val_df["prompt_id"] == prompt_id, "ngram_diversity_score"] = (
                ngram_div
            )
            if prompt_id % 50 == 0:
