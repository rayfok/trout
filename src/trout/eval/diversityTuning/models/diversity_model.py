...
class DiversityModel:
    def __init__(self, modelname, device):
        self.model = SentenceTransformer(
            modelname, device=device, trust_remote_code=True
        )
        self.device = device
    ...
    def get_avg_pairwise_diversity(self, texts, metric="cosine"):
        embeddings = self.get_embedding(texts)
        if metric == "cosine":
            embeddings = F.normalize(embeddings, p=2, dim=1)
            similarity = torch.mm(embeddings, embeddings.T)
        ...
        upper_distances = distance[triu_indices[0], triu_indices[1]]

        return upper_distances.mean().item()
