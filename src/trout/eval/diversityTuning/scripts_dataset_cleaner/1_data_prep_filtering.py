...
parser.add_argument(
    "--data_path",
    default="data/diversityTuning/writingPrompt_cleaned",
    type=str,
...
def remove_long(example):
    ...
    for comment in example["filtered_comment_texts"]:
        ...
        message_templated = tokenizer.apply_chat_template(
    ...
    if len(messages) == 0:
        ...
        example["filtered_comment_texts"] = [
            comment
            for comment, long_comment in zip(
                example["filtered_comment_texts"], long_comments
            )
            if long_comment
        ]
        example["filtered_comment_scores"] = [
            score
            for score, long_comment in zip(
                example["filtered_comment_scores"], long_comments
            )
            if long_comment
        ]
        example["filtered_transformed_scores"] = [
            score
            for score, long_comment in zip(
                example["filtered_transformed_scores"], long_comments
            )
            if long_comment
        ]
    return example
...
def filtering(example):
    ...
    for cur_comment, cur_comment_score, cur_transformed_score in zip(
        cur_comments, cur_comment_scores, cur_transformed_scores
    ):
